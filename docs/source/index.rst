...
pyrpipe allows coding RNA-Seq workflows in pure python in an object oriented manner. 
pyrpipe provides a high level API to many popular RNA-Seq tools for easier and faster development.
...
Depending on the tool, other methods are provided to allow integration and execution with other tools or objects. For example, `perform_alignment()` method of `Hisat2` class 
takes an `SRA` object as input and produces `SAM` files.

pyrpipe makes it intuitive and easy to write RNA-Seq workflows in python. It also provides features like logging, report generation, and benchmarking. 
pyrpipe is not designed to be a full workflow management system and directly manage parallel execution, memory management, and scaling to cluster.  
However, pyrpipe scripts can be easily integrated with workflow management systems like snakemake to achieve more scalable workflows.
